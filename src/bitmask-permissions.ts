 ...
 */
export class BitmaskPermission {
  /**
   ...
   * @public
   */
  constructor(permissionsList: Array<Permissions> = permissions) {
   ...
   * @public
   */
  public getPermissionsList(): Array<Permissions> {
   ...
   * ```
   */
  public validatePermission(permissions: number, permission: string): boolean {
   ...
   * ```
   */
  public assignPermission(permissions: number, permission: string): number {
    const permissionValue = this.getPermissionValue(permission);
   ...
   * ```
   */
  public removePermission(permissions: number, permission: string): number {
    const permissionValue = this.getPermissionValue(permission);
    return permissions & ~permissionValue;
  }
   ...
   * ```
   */
  public getPermissions(permissions: number): string[] {
   ...
   */
  private getPermissionValue(permission: string): number {
    const permissionObj = this.permissionsList.find(
      (p) => p.name === permission
    );
